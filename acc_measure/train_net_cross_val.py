import  numpy as np
import  torch, os
from MiniImagenet_cross_val import MiniImagenet
        ...
        for step, (x_spt, y_spt, x_qry, y_qry) in enumerate(db):
            ...
            if step % 500 == 0 or (step == 10000//args.task_num - 1) & (epoch == range(args.epoch//10000)[-1]):  # evaluation
                ...
                if (step == 10000//args.task_num - 1) & (epoch == range(args.epoch//10000)[-1]):
                    with open('acc/natural2_' + mode_val_test+ '(task_num_' + str(args.task_num) +  ').txt', mode='a') as f:
                        f.write(str(accs[-1]) + ' ' + str(args.cross_val_idx) + ',')
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--cross_val_idx', type=int, help='set this to val idx, TEST INDEX == (val idx + 1) % 10', default=0)
