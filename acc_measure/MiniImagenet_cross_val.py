...
class MiniImagenet(Dataset):
    ...
    def __init__(self, root, mode, batchsz, n_way, k_shot, k_query, resize, cross_val_idx, startidx=0):
        ...
        self.df_spt = pd.read_csv('support_cross_val.csv')
        self.df_qry = pd.read_csv('query_cross_val.csv')
    ...
    def create_batch(self, batchsz, mode):
        ...
        if mode == 'train':
            df_spt = self.df_spt # drop test, val later
            df_qry = self.df_qry # drop test, val later
            df_spt = df_spt[df_spt['cross-val-idx'] != self.cross_val_idx]
            df_spt = df_spt[df_spt['cross-val-idx'] != test_idx]
        elif mode == 'test':
            df_spt = self.df_spt[self.df_spt['cross-val-idx'] == test_idx]
            df_qry = self.df_qry[self.df_qry['cross-val-idx'] == test_idx]
        elif mode == 'val':
            df_spt = self.df_spt[self.df_spt['cross-val-idx'] == self.cross_val_idx]
            df_qry = self.df_qry[self.df_qry['cross-val-idx'] == self.cross_val_idx]
        else: 
            print('===invalid mode===', mode)
        # drop test, val from df_spt

        pres = df_spt['pre'].unique()
#         print(self.cross_val_idx)
