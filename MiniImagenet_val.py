...
class MiniImagenet(Dataset):
    ...
    def __init__(self, root, mode, batchsz, n_way, k_shot, k_query, resize, startidx=0, idx = None):
        ...
        self.cls_num = 2
        self.mode = mode
        self.df_spt = pd.read_csv('support_set_tr_ts_val.csv')
        self.df_qry = pd.read_csv('query_set_tr_ts_val.csv')
    ...
    def create_batch(self, batchsz, mode):
        ...
        if mode == 'train':
            df_spt = self.df_spt[self.df_spt['tr/ts/val'] == 'tr']
            df_qry = self.df_qry[self.df_qry['tr/ts/val'] == 'tr']
        elif mode == 'test':
            df_spt = self.df_spt[self.df_spt['tr/ts/val'] == 'ts']
            df_qry = self.df_qry[self.df_qry['tr/ts/val'] == 'ts']
        elif mode == 'val':
            df_spt = self.df_spt[self.df_spt['tr/ts/val'] == 'val']
            df_qry = self.df_qry[self.df_qry['tr/ts/val'] == 'val']
        else: 
